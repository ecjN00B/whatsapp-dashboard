        ...
        success: async function(result){
            ...
            for(var i=0; i<=result.length; i++) {
                ...
                if(msg == "Audio")
                    ...
                    msg = `<img class='img-thumbnail' src="${src}" />`;
                else
                    msg = `<p>${msg}</p>`;
                ...
                if(result[i].me == 0)
                    $('#conversaConteudo').append(`
                        <div class="containerConv">
                            <img class='rounded' src="${picture}" alt="Avatar" style="width:100%;">
                            ${msg}
    ...
    $('#form1').bind('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                for(var i=0; i<=result.length; i++) {
                    $('#tabela > table > tbody').append("<tr onclick='conversa(\"" + result[i].phone + "\", \"" + result[i].picture + "\")' data-toggle='modal' data-target='#conversa'><td><img class='rounded' src='" + result[i].picture + "' width='50px' height='50px' /></td><td>" + result[i].phone + "</td><td>" + result[i].nick + "</td><td>" + result[i].status + "</td></tr>");
                }
            },
            error: function () {
                alert('Error');
